...
// ---------------------------------------------------------------------------
// Columns. The first "Group" column holds the hierarchical group / sub-group
// names. The grouping dimensions (Team, Role) are NOT columns — the data
// columns are unrelated dummy fields.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Synthetic hierarchy (generated deterministically — no real data logic).
//   Level 0: 30 top-level groups        (grouped by "Team")
//   Level 1: 30 sub-groups per group    (grouped by "Role")
//   Level 2: 50 leaf items per sub-group
// ---------------------------------------------------------------------------
const TOP_GROUPS = 30
const SUBGROUPS = 30
const LEAF_ITEMS = 50
...
const subDims = [
  'Admin', 'Editor', 'Viewer', 'Manager', 'Analyst', 'Owner',
]
const firstNames = [
...
// ---------------------------------------------------------------------------
// Icons (Polar UI style, 16px) — self-contained scenario file.
// ---------------------------------------------------------------------------
function FilterIcon() {
  return (
...
// ---------------------------------------------------------------------------
// Micro pagination — compact copy of the main pagination.
//   density: 'sm' (-1) for every nested level (no per-level step-down)
//   tone: applied to the full row via the cell — 1 (-1 backfill) sub-groups,
//         2 (-2 backfill) leaf items
//   summaryWidth: caps the left summary to the group column it addresses
// ---------------------------------------------------------------------------
...
export default function RowGrouping() {
  // Header chrome (visual parity with Basic Table — cosmetic, not wired into
  // the synthetic hierarchy).
  const [sort, setSort] = useState({ key: null, dir: 'asc' })
  const [openFilters, setOpenFilters] = useState({})
  const [filters, setFilters] = useState({})

  // Main pagination pages the 30 top-level groups.
  ...
  const setMicro = (key, value, count) => {
    const next = Math.min(Math.max(1, value), count)
    setMicroPage((prev) => ({ ...prev, [key]: next }))
  }
  ...
  return (
    <main className="dt-scenario dt-scenario--fill">
      <div className="dt-topbar">
        ...
        <h1 className="dt-title">Row Grouping — Micro Pagination</h1>
        <p className="dt-subtitle">
          Fixed-height table with sticky header, sticky micro-pagination, and a
            ...
            <colgroup>
              ...
              {dataColumns.map((col) => (
                <col
                  key={col.key}
                  style={{
                    width: `calc((100% - ${GROUP_COL_WIDTH}px) / ${dataColumns.length})`,
                  }}
                />
              ))}
            </colgroup>
                  ...
                  <Fragment key={gKey}>
                    ...
                    {gOpen && (
                      <>
                            ...
                            <Fragment key={sKey}>
                              <tr className="dt-group-row" data-level="1">
                                <td className="dt-group-cell">
                                  <div
                                    className="dt-group-inner"
                                    style={{ paddingLeft: 12 + INDENT_STEP }}
                                  >
                                    <button
                                      type="button"
                                      className="dt-group-toggle"
                                    ...
                                    <span className="dt-group-value">
                                      {subLabel(si)}
                                    </span>
                                    <span className="dt-group-count">
                                      {LEAF_ITEMS}
                                    </span>
                                  </div>
                                </td>
                                {dataColumns.map((col) => (
                                  <td
                                    key={col.key}
                                    className="dt-cell dt-cell--muted"
                                  />
                                ))}
                              </tr>
                              ...
                              {sOpen && (
                                <>
                                    ...
                                    (_, iidx) => {
                                      ...
                                      return (
                                        <tr className="dt-row" key={item.id}>
                                          <td
                                            className="dt-cell dt-cell--group-spacer"
                                            style={{
                                              paddingLeft: 12 + INDENT_STEP * 2,
                                            }}
                                          />
                                          {dataColumns.map((col) => (
                                  ...
                                  {/* Leaf-level micro pagination (tone -2, density -1) */}
                                  <tr className="dt-micro-row">
                                    <td
                                      className="dt-micro-cell is-leaf dt-micro-tone-2"
                                      colSpan={columns.length}
                                    >
                                      <div
                                        className="dt-micro-indent"
                                        style={{ paddingLeft: 12 + INDENT_STEP * 2 }}
                                      >
                                        <MicroPagination
                                          density="sm"
                                          label="Items"
                                          page={sMicro}
                                          pageCount={leafMicroCount}
                                          total={LEAF_ITEMS}
                                          summaryWidth={
                                            GROUP_COL_WIDTH - (12 + INDENT_STEP * 2)
                                          }
                                          onGoTo={(p) =>
                                            setMicro(sKey, p, leafMicroCount)
                                          }
                                        />
                                      </div>
                                    </td>
                        ...
                        {/* Sub-group-level micro pagination (tone -1, density -1) */}
                        <tr className="dt-micro-row">
                          <td
                            className="dt-micro-cell dt-micro-tone-1"
                            colSpan={columns.length}
                          >
                            <div
                              className="dt-micro-indent"
        ...
        <div className="dt-footer">
          ...
          <div className="dt-pagination">
            ...
            <button
              ...
              aria-label="Previous page"
              disabled={currentPage === 1}
              onClick={() => goToMain(currentPage - 1)}
            >
              <ChevronLeft />
            </button>
            ...
            <button
              ...
              className="dt-page-btn"
              aria-label="Next page"
              disabled={currentPage === topPageCount}
              onClick={() => goToMain(currentPage + 1)}
            >
              <ChevronRight />
            </button>
